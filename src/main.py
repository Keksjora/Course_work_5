...
from src.api_hh import get_employee_data, get_vacancies, get_vacancy_list
from src.DBManager import DBManager
from src.utils import create_data_base, save_data_to_db

params = config()
data = get_vacancies(get_employee_data())
...
def interfaсe():
    ...
    if user_answer == "1":
        ...
        print(f"Средняя зарплата по вакансиям: {vacancies_with_higher_salary}\n")
    elif user_answer == "5":
        user_input = input("Введите слово: ")
        vacancies_with_keyword = db_manager.get_vacancies_with_keyword(user_input)
        print(
            f"Список всех вакансий, в названии которых содержатся запрашиваемое слово: {vacancies_with_keyword}"
        )
    else:
